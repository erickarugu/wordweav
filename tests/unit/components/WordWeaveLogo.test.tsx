...
describe("WordWeaveLogo Component", () => {
  ...
  test("renders compact version", () => {
    ...
    // Compact version still shows text, just in a more condensed format
  ...
  test("has unique gradient IDs to avoid conflicts", () => {
    ...
    const gradients1 = Array.from(
      container1.querySelectorAll("linearGradient")
    ).map((g) => g.id);
    const gradients2 = Array.from(
      container2.querySelectorAll("linearGradient")
    ).map((g) => g.id);
  ...
  test("shows text in full version", () => {
    ...
    // Should show WordWeave text in full version (may have multiple for shadow effect)
    const textElements = screen.getAllByText("WordWeave");
