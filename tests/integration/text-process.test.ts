...
import { createMockUser, createMockDocument, createMockAiResponse } from '../utils/testHelpers'
...
const mockGetAIService = getAIService as jest.MockedFunction<typeof getAIService>
...
describe('/api/text/process', () => {
  ...
  describe('POST', () => {
    ...
    test('falls back to basic processing if AI fails', async () => {
      ...
      const mockAiService = {
        processText: jest.fn().mockRejectedValue(new Error('AI service unavailable')),
      }
      mockGetAIService.mockReturnValue(mockAiService)
